...
/**
 * Update Job Seeker Profile Action
 * Updates the authenticated user's job seeker profile
 */
export  const updateProfileAction =  actionClient
  .schema(updateProfileSchema)
  .action(async ({ parsedInput: data }) => {
    try {
      console.log('[Update Profile Action] ========== STARTING PROFILE UPDATE ==========');
      console.log('[Update Profile Action] Data keys:', Object.keys(data));
      console.log('data', data)
      // Call the update profile API
      const response = await jobSeekerRepository.updateProfile(data as IUpdateProfileRequest);
      console.log('response', response)

      if (!response.profile) {
        console.error('[Update Profile Action] Update failed');
        throw new ActionError(response.message || 'Failed to update profile');
      }

      // Revalidate the profile cache tag to refetch data on next request
      // In Next.js 15+, revalidateTag requires a second parameter (cache profile)
      revalidateTag("job-seeker-profile", "default");
      ...
      console.error('[Update Profile Action] ========== EXCEPTION ==========');
      console.error('[Update Profile Action] Error:', error);

      // Extract more detailed error information
      if (error && typeof error === 'object' && 'info' in error) {
        console.error('[Update Profile Action] Error info:', error.info);
        const errorInfo = error.info as Record<string, string>;

        // Check if there's a message in the error info
        if (errorInfo && typeof errorInfo === 'object' && 'message' in errorInfo) {
          console.error('[Update Profile Action] API Error Message:', errorInfo.message);
          throw new ActionError(errorInfo.message || 'Failed to update profile');
        }
      }

      if (error instanceof ActionError) throw error;
      throw new ActionError('Failed to update profile. Please try again.');
    }
  });
