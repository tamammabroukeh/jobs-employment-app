...
import { Typography, ReusableButton, ReusableDialog, ReusableSelect, ReusableInput } from '@/components/Reusable-Components';
...
export default function ApplyJobDialog({ jobId, jobTitle, open, onClose }: ApplyJobDialogProps) {
  ...
  const noticePeriodOptions = [
    { title: t('applyDialog.noticePeriodOptions.immediate'), value: 'immediate' },
    ...
    { title: t('applyDialog.noticePeriodOptions.twoMonths'), value: 'twoMonths' },
  ];
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div>
          <label className="block mb-3">
            <Typography variant="small" className="text-foreground font-medium">
              {t('applyDialog.currentStatus')} <span className="text-destructive">*</span>
            </Typography>
          </label>
          <Controller
            name="currentStatus"
            ...
            render={({ field }) => (
              <Radio.Group {...field} className="w-full">
                <Radio value="working">{t('applyDialog.statusWorking')}</Radio>
                <Radio value="notWorking">{t('applyDialog.statusNotWorking')}</Radio>
              </Radio.Group>
            )}
          />
        ...
        <div>
          <label className="block mb-2">
            <Typography variant="small" className="text-foreground font-medium">
              {t('applyDialog.noticePeriod')} <span className="text-destructive">*</span>
            </Typography>
          </label>
          <Controller
            name="noticePeriod"
        ...
        <div>
          <label className="block mb-2">
            <Typography variant="small" className="text-foreground font-medium">
              {t('applyDialog.expectedSalary')} <span className="text-destructive">*</span>
            </Typography>
          </label>
              ...
              <Controller
                ...
                rules={{
                  ...
                  validate: (value, formValues) =>
                    value >= formValues.salaryFrom || 'Must be greater than minimum salary',
                }}
                render={({ field }) => (
                  <ReusableInput
                    {...field}
        ...
        <div className="flex gap-3 justify-end pt-4">
          <ReusableButton variant="default" onClick={handleClose} disabled={isSubmitting}>
          ...
          >
            {isSubmitting ? t('applyDialog.applying') : t('applyDialog.confirmApply')}
          </ReusableButton>
        </div>
      </form>
    </ReusableDialog>
